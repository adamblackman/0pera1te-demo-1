    ...
    <div className="p-6 space-y-4 border rounded-lg">
      ...
      <div
        ...
        className={`
          ...
          ${dragOver ? "scale-[1.02] bg-muted border-primary" : "border-border"}`
        }
      >
        {workflow.length === 0 ? (
          <span className="text-sm w-full text-center text-muted-foreground">
          ...
          workflow.map((chip, index) => (
            <div key={chip.id} className="flex items-center">
              ...
              {index < workflow.length - 1 && (
                <ChevronRight
                  className={`w-4 h-4 mx-1 flex-shrink-0 text-primary ${isChaos ? "animate-pulse" : ""}`}
                />
              )}
            </div>
          ))
      ...
      {workflow.length > 0 && (
        <div
          className="text-xs text-center py-2 rounded-lg bg-muted text-muted-foreground"
        >
